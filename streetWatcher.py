...
def notIn (location, locations):
	...
	if locations != None:
		...
		for actual in locations :
			...
			print(math.fabs(float(actual["latitude"].encode('ascii')) - float(location[1].encode('ascii'))) < 0.50)
			print(math.fabs(float(actual["latitude"].encode('ascii')) - float(location[1].encode('ascii'))))
